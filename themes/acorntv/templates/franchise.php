...
if(count($_POST) > 0 && isset($_COOKIE["ATVSessionCookie"], $_POST['action']) && rljeApiWP_isUserActive($_COOKIE["ATVSessionCookie"])) :
    ...
    if (function_exists('rljeApiWP_getFranchiseById')) :
        ...
        if (isset($franchise->id)) :
                    ...
                    foreach($franchise->seasons as $season) {
                        foreach($season->episodes as $keyEpisode=>$episode) {
                            if($episode->id === $streamPositionKeys[0]) {
                                ...
                                $franchiseArtTitle = 'Click to continue watching '.$season->name.': Episode '.($keyEpisode+1);
                                $break = true;
                                break;
                            }
                        }
                    ...
                    foreach($franchise->seasons as $season) {
                        foreach($season->episodes as $keyEpisode=>$episode) {
                            ...
                            if($isNextEpisode && $isResume) {
                                $franchiseArtLink = $franchiseURL.'/'.rljeApiWP_convertSeasonNameToURL($season->name).'/'.rljeApiWP_convertEpisodeNameToURLFriendly($episode->name);
                                $franchiseArtTitle = 'Click to watch '.$season->name.': Episode '.($keyEpisode+1);
                                $break = true;
                                break;
                            }
                            if(!$isNextEpisode) {
            ...
            else if(isset($franchise->episodes[0], $franchise->episodes[0]->id) && is_numeric($franchise->episodes[0]->id)) {
    ...
    <div class="secondary-bg" style="padding-bottom:50px">
        <div class="container franchise">
            <h4 class="subnav">
                ...
                <span itemprop="name"><?= $franchise->name; ?></span>
            ...
            <div class="row" >
                <div class="col-xs-12 col-sm-6 col-lg-4">
                    ...
                    <a href="<?= $franchiseArtLink; ?>" class="js-play-resume">
                        <img itemprop="image" class="wp-post-image" id="franchise-avatar" title="<?= $franchiseArtTitle ?>" src="https://api.rlje.net/acorn/artwork/size/<?= $franchise->image; ?>?w=460" />
                    </a>
                    <?php else : ?> 
                    <img itemprop="image" class="wp-post-image" id="franchise-avatar" src="https://api.rlje.net/acorn/artwork/size/<?= $franchise->image; ?>?w=460" />
                    <?php endif; ?>
                </div>
                <div class="col-xs-12 col-sm-6 col-lg-8" >
                    <p id="franchise-description" itemprop="description"><?= $franchise->longDescription;?></p>
                    <?php 
                    ...
                    ?>
                    <a class="inline"><button id="watchlistActionButton" onclick="addToWatchlist('<?= $franchise->id; ?>')">Add to Watchlist</button></a>
                    <?php   else: ?>
                    <a class="inline"><button id="watchlistActionButton" onclick="removeFromWatchlist('<?= $franchise->id; ?>')">Remove from Watchlist</button></a>
                    <?php   endif; ?>
                    <?php
                        endif;
                    ?>
                    ...
                    <span itemprop="trailer" itemscope itemtype="http://schema.org/VideoObject">
                        <meta itemprop="thumbnailUrl" content="https://api.rlje.net/acorn/artwork/size/<?= $franchise->image; ?>?w=750" />
                        ...
                        <a class="inline" href="<?= $franchiseURL.'/trailer'; ?>">
