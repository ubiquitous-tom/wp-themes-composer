...
class GFFormSettings {
	...
	public static function confirmations_list_page( $form_id ) {
		...
		$add_new_url = esc_url( add_query_arg( array( 'cid' => 0 ) ) );
		?>
			...
			<div id="gform_tab_group" class="gform_tab_group vertical_tabs">
				...
				foreach ( $setting_tabs as $tab ) {
					...
					<li <?php echo $current_tab == $tab['name'] ? "class='active'" : '' ?>>
						<a href="<?php echo esc_url( add_query_arg( $query ) ); ?>"><?php echo $tab['label'] ?></a><span></span>
					</li>
				<?php
					}
					?>
	...
	public static function handle_confirmation_edit_submission( $confirmation, $form ) {
		...
		if ( $result !== false ) {
			GFCommon::add_message( sprintf( __( 'Confirmation saved successfully. %sBack to confirmations.%s', 'gravityforms' ), '<a href="' . esc_url(remove_query_arg( array( 'cid', 'duplicatedcid' ) )) . '">', '</a>' ) );
		} else {
...
class GFConfirmationTable extends WP_List_Table {
	...
	function column_name( $item ) {
		$edit_url      = esc_url( add_query_arg( array( 'cid' => $item['id'] ) ) );
		$duplicate_url = esc_url( add_query_arg( array( 'cid' => 0, 'duplicatedcid' => $item['id'] ) ) );
