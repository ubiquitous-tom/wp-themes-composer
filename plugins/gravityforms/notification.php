...
Class GFNotification {
	...
	public static function notification_edit_page( $form_id, $notification_id ) {
		...
		if ( $is_update && $is_valid ) {
			GFCommon::add_message( sprintf( __( 'Notification saved successfully. %sBack to notifications.%s', 'gravityforms' ), '<a href="' . esc_url( remove_query_arg( 'nid' ) ) . '">', '</a>' ) );
	...
	public static function notification_list_page( $form_id ) {
		...
		GFFormSettings::page_header( __( 'Notifications', 'gravityforms' ) );
		$add_new_url = esc_url( add_query_arg( array( 'nid' => 0 ) ) );
		?>
...
class GFNotificationTable extends WP_List_Table {
	...
	function column_name( $item ) {
		$edit_url = esc_url( add_query_arg( array( 'nid' => $item['id'] ) ) );
	...
	function no_items() {
		...
		printf( __( "This form doesn't have any notifications. Let's go %screate one%s.", 'gravityforms' ), "<a href='" . esc_url( add_query_arg( array( 'nid' => 0 ) ) ) . "'>", '</a>' );
	}
}