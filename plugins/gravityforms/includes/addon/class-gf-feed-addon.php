...
abstract class GFFeedAddOn extends GFAddOn {
	...
	public function form_settings_header() {
		if ( $this->is_feed_list_page() ) {
			...
			return $title . " <a class='add-new-h2' href='" . esc_url(add_query_arg( array( 'fid' => 0 ) )) . "'>" . __( 'Add New', 'gravityforms' ) . '</a>';
		}
	}
	...
	protected function get_action_links() {
		$feed_id  = '{id}';
		$edit_url = esc_url( add_query_arg( array( 'fid' => $feed_id ) ) );
		$links    = array(
	...
	public function feed_list_no_item_message() {
		return sprintf( __( "You don't have any feeds configured. Let's go %screate one%s!", 'gravityforms' ), "<a href='" . esc_url( add_query_arg( array( 'fid' => 0 ) ) ) . "'>", '</a>' );
	}
