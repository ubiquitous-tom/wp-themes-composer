(function($){
		...
		$('.carousel-pagination-slide').on('slid.bs.carousel', function(carousel) {
								...
								if(0 === pagedItem.length && data !== null) {
										var currentPg = data.currentpage,
												...
												addNewItems = function(index, value){
														...
														for(var i=0; i<4; i++) {
																var imageURL = browse_object.image_url + value.franchiseID + '_avatar?w=400&h=225',
																		href = browse_object.home_url + '/' + value.franchiseID;

																imageURL = $(document).triggerHandler('carousel_pagination.add_new_item_image_url', [ imageURL, value ]);
																if(3 > i) {
												...
												completeCarousel = function(items) {
														...
														for(var i=0; i<3 ; i++) {
																newItems.push({
																		'franchiseID': $carouselListItems.eq(i).find('a').attr('href').replace(browse_object.home_url+'/', '')
																});
														}
														return newItems;
												};
												...
												for(var i = 5, j=0; i <= 7 ; i++,j++) {
														for(var x = 0; x<=j; x++){
																var $carouselItem = $carouselListItems.eq(i),
																		...
																		imageURL = browse_object.image_url + '{{ID}}_avatar?w=400&h=225',
																		href = browse_object.home_url + '/{{ID}}',
																		ID = items[j-x].franchiseID;

																imageURL = $(document).triggerHandler('carousel_pagination.second_page_image_url', [ imageURL, items[j-x] ]);
																$carouselChangeItem
																	.find('a').attr('href', href.replace('{{ID}}', ID))
				...
				if((loadPage || isLatestItem) && nextPage <= totalPages) {
						data.page = nextPage;
						$.post(browse_object.ajax_url, data, processData).fail(function(error) {
