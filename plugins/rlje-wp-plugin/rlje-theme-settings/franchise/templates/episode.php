	...
	<div class="container franchise">
		...
		<h4 class="subnav">
			...
			if ( 'movie' === strtolower( $episode->type ) ) {
				...
				echo '<span itemprop="partOfSeason">' . $season->name . '</span> : <span itemprop="name">' . esc_html( ( strlen( $episode->name ) > 45 ) ? substr( $episode->name, 0, 45 ) . '...' : $episode->name ) .'</span>   <!-- Next link -->';
			}
			?>
